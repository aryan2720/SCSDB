...
const TvDetails = () => {
  ...
  useEffect(()=>{
    // fetch tv details using tv id - yahan fetch hora h data 
    dispatch(asyncLoadTv(id))
    return ()=>{
      //component will unmounting hua hume useEffect ke andar
      //dispatch ki tv ki info null hua h
      dispatch(removeTv())
    }
  },[id])
   //tv li details aap yahan pe access kroge redux k store k andr reducers se
  ...
  return info?(
    <div style={{
      ...
      <div className='w-full mt-[2vw] flex gap-[1vw]'>
        ...
        <div className='w-[70%] px-[1vw]'>
            <div className='flex items-center gap-[.5vw]'>
              ...
              <small className='text-[1.4vw] text-zinc-200 font-regular tracking-tighter'>({info.details.first_air_date.split("-")[0]})</small>
              </h1>
            ...
            <div className='flex items-center gap-[.3vw]'>
              <h1 className='text-zinc-100 text-md tracking-tighter'>Episodes: {info.details.number_of_episodes}</h1>
            </div>
      ...
      <div className='w-full h-fit mt-[1vw]'>
        <h1 className='text-white tracking-tight font-semibold mb-[.5vw]'>Seasons</h1>
        <HorizontalCards data={info.details.seasons.length>0 && info.details.seasons} category={category} />
      </div>
